...
\textheight=20cm      % чтобы равенства уместились
\oddsidemargin=-0.7cm % на странице
...
$\varphi_n$ -- любой из функций $f_n, g_n, h_n, t_n$ верно:
$$l(\pphi{n}) \geqslant \frac{2}{5} \cdot 5^n$$
\end{mylm}
\begin{proof}
Из таблицы \ref{functab3} видно, что при $n=1,\ l(\pphi{1}) \geqslant 2$.
...
функции типа $\varphi_{n-1}$ или через $4$ функции типа $\varphi_{n-1}$ и одну из функций
$\{s^2_{n-1}, s^3_{n-1}\}$, поэтому
$${l(\pphi{n})\geqslant\min(\frac{2}{5}\cdot5^{n-1}\cdot5,\frac{2}{5}\cdot5^{n-1}\cdot4+1\cdot5^{n-1})} = \frac{2}{5}\cdot5^n.$$
\end{proof}
...
$x_{n-m+1}$ имеет поляризацию $4$ и $\varphi_{n-m+1}$ выражается через $4$ функции типа
$\varphi_{n-m}$ и одну функцию $s^3_{n-m}$, причем по теореме \ref{ths3}
